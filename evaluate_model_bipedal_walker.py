...
num_episodes = 0
timesteps = 0
print_interval = 100  # 每 100 个时间步打印一次
...
while num_episodes < 1:
    ...
    current_obs_for_print = obs # 保存当前步的 obs 用于打印
    ...
    if timesteps % print_interval == 0:
        if isinstance(current_obs_for_print, np.ndarray) and current_obs_for_print.ndim == 2:
            param1 = current_obs_for_print[0, 0] # Hull angle
            param2 = current_obs_for_print[0, 1] # Hull angular velocity
            param3 = current_obs_for_print[0, 2] # Velocity x
            param4 = current_obs_for_print[0, 3] # Velocity y
    ...
    if dones.any():
        ...
        # 判断结束原因 (基于步数和奖励)
        if final_steps < 1600 and final_reward > 200: # 假设高分+未满步数=到达终点
...
env.close()
# print(f"模型 {model_path} 评估完成。") # 这句可以注释掉或删除，因为信息已在循环内打印 