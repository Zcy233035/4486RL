...
def main():
    ...
    # 添加 --device 参数，默认为 auto
    parser.add_argument("--device", type=str, default="auto", choices=["auto", "cpu", "cuda"],
                        help="Device to use for training (auto, cpu, cuda). 'auto' detects CUDA.")
    ...
    # 注意：在服务器上通常没有图形界面，render_mode="human" 可能会报错或无效
    # 如果你需要在服务器评估，可以去掉 render_mode 或设为 None
    eval_env = gym.make(env_id) # 评估时通常不需要渲染，移除 render_mode="human"
    # 使用包含 .zip 的路径加载模型，加载时也指定设备
    ...
    # 让模型跑一个 episode 看看效果 (在服务器上演示通常意义不大，可以注释掉或移除)
